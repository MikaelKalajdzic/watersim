Initialization of the scene, camera, renderer, and controls
``` javascript
...
Creation of a water surface using a grid of points
``` javascript
...
scene.add(mesh);
```
Add reflexion to the water surface
``` glsl
...
vec3 finalColor = Ka * ambientColor +
                  ...
                  reflectionColor;
```
Simulation of the water surface
``` javascript
...
```
Generating a ripple disturbance
``` javascript
...
```
The functions in charge of the rain
``` javascript
...
```
Add mouse controls to generate ripple on click
``` javascript
// Create a raycaster
